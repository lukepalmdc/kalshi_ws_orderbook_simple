...
class KalshiHttpClient(KalshiBaseClient):
    ...
    def place_order(self, order_params: Dict[str, Any]) -> Dict[str, Any]:
        """Retrieves the account balance."""
        return self.post(self.portfolio_url + "/orders", body=order_params)
...
class KalshiWebSocketClient(KalshiBaseClient):
    ...
    async def on_open(self):
        ...
        #await self.subscribe_to_markets(["orderbook_delta"], ["KXNHLGAME-25DEC19WPGCOL-COL", "KXNHLGAME-25DEC19WPGCOL-WPG"])
    ...
    async def resubscribe(self, market_tickers):
        """Re-subscribe to channels and markets, e.g., after a gap or reconnect."""
        print("Re-subscribing to channels: markets:", market_tickers)
        # Optionally, send an unsubscribe first
        unsubscribe_message = {
            "id": self.message_id,
            "cmd": "unsubscribe",
            "params": {
